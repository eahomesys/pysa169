...
class MemberViewController: UIViewController, MenuControllerDelegate {
    ...
    func loadMessages() {
        ...
        if (filter == "Address") {
            let query = collection.order(by: K.FStore.addressField).order(by: K.FStore.aptField)
            getMemberData(query: query)
        } else if (filter == "Status") {
            let query = collection.whereField(K.FStore.Status, isEqualTo: "Past")
            getMemberData(query: query)
        } else if (filter == "Calling") {
            let query = collection.whereField(K.FStore.Calling, isEqualTo: "-")
            getMemberData(query: query)
        } else {
    ...
    func getMemberData(query: Query){
